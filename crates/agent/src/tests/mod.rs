...
fn wait_for_running_subagent(thread: &Entity<Thread>, cx: &mut TestAppContext) -> acp::SessionId {
    for _ in 0..10 {
        cx.run_until_parked();
        if let Some(session_id) = thread.read_with(cx, |thread, cx| {
            thread.running_subagent_ids(cx).into_iter().next()
        }) {
            return session_id;
        }
    }
...
async fn test_subagent_tool_call_end_to_end(cx: &mut TestAppContext) {
    ...
    let subagent_session_id = wait_for_running_subagent(&thread, cx);
...
async fn test_subagent_error_propagation(cx: &mut TestAppContext) {
    ...
    let _subagent_session_id = wait_for_running_subagent(&thread, cx);
    ...
    wait_for_no_running_subagents(&thread, cx, "subagent should not be running after error");
